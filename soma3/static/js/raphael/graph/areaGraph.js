...
function newAreaGraph(file, objName, att)
{
	...
	{
		var info = [];
		info.width					= checkAB("width", "obj.width()");
		info.height 				= checkAB("height", "obj.height()");
		info.lineWidth				= checkAB("lineWidth", "3");
		info.leftgutter 			= checkAB("leftgutter", "30");
		info.bottomgutter 			= checkAB("bottomgutter", "20");
		info.topgutter 				= checkAB("topgutter", "20");
		info.horizonLine 			= checkAB("horizonLine", "true");
		info.verticalLine 			= checkAB("verticalLine", "true");
		info.color 					= checkAB("color", "\"hsl(\" + [.6, .5, .5] + \")\" ");
		info.lineColor 				= checkAB("lineColor", "\"#000\" ");
		info.textColor				= checkAB("textColor", "\"#fff\" ");
		info.tooltip				= checkAB("tooltip", "true");
		info.tooltipStyle			= checkAB("tooltipStyle", "{fill: \"#000\", stroke: \"#666\", \"stroke-width\": 2, \"fill-opacity\": .7}");
		info.tooltipValueText		= checkAB("tooltipValueText", "changeValueText");
		info.tooltipKeyText			= checkAB("tooltipKeyText", "changeKeyText");
		
		if(tWidth != null)
		{
			if(tWidth == info.width && tHeight == info.height)
				return;
		}
		tWidth = info.width;
		tHeight = info.height;
		...
		obj.html("");
		complateDataLoad(tempData, info);
	...
	function complateDataLoad(dataa, info)
	{
		// Grab the data
		var labels = [],
			data = [];

		var response = $.parseJSON(dataa);
